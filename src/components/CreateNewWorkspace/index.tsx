...
const CreateNewWorkspace = ({ visible, onClose }: any) => {
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    const data = { name: `${workspaceName}`, color: `${selectedColor}` };
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <button onClick={handleBackBtn}>
              {stage > 1 && <Icon iconName="LeftArrow" />}
            </button>
          </div>
          {/* first stage*/}
          {stage === 1 && (
          ...
          {stage === 2 && (
            <div className="mb-8 mt-8 flex">
              <div
                className={`w-[70px] h-[70px] ml-[10px] rounded-lg text-white text-2xl font-black flex justify-center items-center`}
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`relative ${
                        color === selectedColor
                          ? "h-7 w-7 rounded-xl"
                          : "h-5 w-5 m-[5px] rounded-lg"
                      } rounded-lg ${color === "#FD7E14" ? "break-line" : ""}`}
          ...
          {stage === 3 && (
            <div className="mb-8 mt-8 pt-4 pb-4 pr-3 pl-3 flex-col justify-between border-[#AAAAAA] rounded-lg border-[0.5px]">
              ...
              <div className="flex justify-between h-[34px]">
                ...
                <div
                  className={`h-[15px] w-[15px] m-[5px] rounded-sm`}
                  style={{ backgroundColor: `${selectedColor}` }}
                ></div>
              </div>
            </div>
